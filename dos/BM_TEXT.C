...
static Pixmap	pmchar1;			/* pixmap des caracteres */
static Pixmap	pmchar2;			/* pixmap des caracteres */

static char		*pchaine;			/* pointe chaine editee */
static short	lgchaine;			/* longueur de la chaine */
...
static char		ifcx;				/* 1 -> curseur "|" present */
static short	cx;					/* position de curseur "|" en x */
static short	begin;				/* debut ou afficher [car] */
static short	charsize;			/* taille des caracteres */
...
/*
	Retourne la largeur d'un caractere.
 */
...
/*
	Dessine un caractere et avance la position pour le suivant.
 */
...
/*
	Dessine un caractere accentue ou autre.
 */
...
void DrawAccent (Pixmap *ppm, Pt *ppos, char c, ShowMode mode)
{
	...
	{
		if ( c == '\n' )  c = 127;					/* petit triangle ">" */
		DrawChar(ppm, ppos, c, mode);				/* dessine le caractere */
	}
}
...
/*
	Dessine une chaine de caracteres terminee par zero.
	Retourne la position suivante, sur la ligne de base.
		*ppm		->	pixmap ou dessiner (0 = ecran)
		pos			->	depart sur la ligne de base
		pstring		->	chaine terminee par zero
		size		->	taille des caracteres
...
Pt DrawText (Pixmap *ppm, Pt pos, char *pstring, short size, ShowMode mode)
{
	...
	while ( (c = *pstring++, c != 0) )
	{
		DrawAccent(ppm, &pos, c, mode);				/* dessine le caractere */
	}
...
/*
	Retourne le rectangle englobant une chaine de caracteres.
...
/*
	Donne les caracteristiques du mot suivant.
	Retourne la longueur a utiliser.
 */
...
/*
	Dessine un paragraphe de texte dans un rectangle, en drapeau a droite.
 */
...
void DrawParagraph (Pixmap *ppm, Rectangle rect, char *pstring, short size, ShowMode mode)
{
	...
	if ( *pstring != 0 )						/* texte pas entierement affiche ? */
	{
...
/*
	Effectue quelques calculs pour afficher une ligne en edition.
 */
...
void CalcJustif (void)
{
	...
	cx  = chrect.p1.x;						/* curseur "|" au debut */
	
	/*	Cherche dans la chaine le premier caractere a afficher. */
	...
	/*	Cherche le nombre de caracteres et la longueur a afficher. */
...
/*
	Affiche un texte pendant l'edition d'une ligne.
 */
...
void EditAff (void)
{
	...
	for ( i=begin ; i<lgchaine ; i++ )
	{
		...
		DrawAccent(0, &pos, pchaine[i], MODELOAD);		/* affiche un caractere */
	}
...
/*
	Insere un caractere dans la ligne.
 */
...
short InsChar (char car)
{
	...
	if ( lgchaine >= lgmax-1 )  return -1;	/* chaine pleine */
	...
	*(pchaine+curseur) = car;				/* met le caractere dans la chaine */
	lgchaine ++;
...
/*
	Detruit un caractere dans la ligne.
 */
...
short DelChar (void)
{
	...
	if ( lgchaine == 0 )  return -1;		/* chaine vide */
	if ( curseur == 0 )  return -1;			/* curseur au debut */
...
/*
	...
	Si oui, retourne l'accent. Si non, retourne zero.
 */
...
/*
	...
	Si oui, retourne le nouveau caractere. Si non, retourne zero.
 */
...
/*
	Enleve le curseur "|" si necessaire.
 */
...
/*
	Modifie la chaine en edition selon un evenement clavier/souris.
	Retourne -1 en cas d'erreur.
	Retourne 1 si l'evenement ne modifie pas le contenu.
	Retourne 0 si l'evenement modifie le contenu.
 */
...
short EditEvent (short key, Pt pos)
{
	...
	{
		err = DelChar();				/* detruit un caractere */
	}
	...
	if ( ifaccent )						/* accent flottant en cours ? */
	{
		...
		if ( ifaccent )
		{
			...
			key = ifaccent;				/* remplace par lettre accentuee */
			goto ins;
		}
	}
	...
	if ( ifaccent )
	{
		key = ifaccent;					/* insere l'accent flottant */
	}
	...
	if ( key >= 32 && key <= 127 ||
		 ...
		 key == '\n' )
	{
		err = InsChar((char)key);		/* insere le caractere frappe */
	}
...
/*
	Initialise la chaine a editer, et place le curseur a la fin.
 */
...
/*
	Termine la chaine a editer, et efface le curseur.
 */
...
short EditClose (void)
{
	...
	curseur = 0;					/* met le curseur au debut */
	
	EditDraw();						/* affiche la chaine */
	ClrCurs();						/* enleve le curseur "|" */
	
	pchaine = 0;					/* y'a plus de chaine en edition */
