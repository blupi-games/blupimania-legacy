...
#ifdef __MSDOS__
#define STEPDEL	2			/* delai d'un pas (unite 20ms) */
#define STEPX	24			/* pas horizontal (deplacement total = 176) */
#define STEPY	16			/* pas vertical   (deplacement total =  90) */
#else
#define STEPDEL	3			/* delai d'un pas (unite 20ms) */
#define STEPX	24			/* pas horizontal (deplacement total = 176) */
#define STEPY	18			/* pas vertical   (deplacement total =  90) */
#endif
...
/*
	Fabrique l'image de fond pour le decor en mixant l'ancien pixmap
	avec le nouveau lors d'un decalage horizontal de gauche a droite.
 */
...
#define NBDIVISIONS 18
	...
	dim1.y = DIMYDRAW / NBDIVISIONS ;
	dim1.x = part ;

	dim2.y = DIMYDRAW / NBDIVISIONS ;
	dim2.x = DIMXDRAW - part ;

	  os1.y = 0 ;
	  os1.x = DIMXDRAW-total ;
	  od1.y = POSYDRAW ;
	  od1.x = POSXDRAW+DIMXDRAW-part ;
	...
	for (i = 0 ; i < NBDIVISIONS ; i++)
	{
	  ...
	  CopyPixel (ppmnew, os1, 0, od1, dim1, MODELOAD) ;

	  CopyPixel (ppmold, os2, 0, od2, dim2, MODELOAD) ;
	  os1.y += TAILLEY ;
	  os2.y += TAILLEY ;
	  od1.y += TAILLEY ;
	  od2.y += TAILLEY ;
...
/*
	Fabrique l'image de fond pour le decor en mixant l'ancien pixmap
	avec le nouveau lors d'un decalage horizontal de droite a gauche.
 */
...
/*
	Fabrique l'image de fond pour le decor en mixant l'ancien pixmap
	avec le nouveau lors d'un decalage vertical de haut en bas.
 */
...
/*
	Fabrique l'image de fond pour le decor en mixant l'ancien pixmap
	avec le nouveau lors d'un decalage vertical de bas en haut.
 */
...
void showmap (Pixmap *p, char *t)
{
  ...
  CopyPixel (p, os, 0, os, dim, MODELOAD) ;
  gotoxy (1,1) ;
  printf (t) ;
  getch () ;
  blackscreen () ;
...
void DecorSetOrigine (Pt origine, short quick)
{
	...
	else
	{
		...
		IconDrawClose(1);						/* enleve la super cellule de l'ecran */
		...
		ovisu = origine;
		DecorMake(0);							/* pmdecor <-- adapte le decor */
		...
		MoveRedraw();							/* redessine sans changement */
		IconDrawClose(0);						/* pmwork <-- icones (nouveau) */
		ppmicon = IconGetPixmap();				/* ppmicon <-- pmwork */
		showmap (ppmicon, "HELLO") ;
	
		err = SavePixmap(&pmdecor);				/* sauve le nouveau decor */
		if ( err != 0 )  return;

		CopyPixel								/* pmdecor <-- ecran (actuel) */
		(
			0,        (p.y=POSYDRAW, p.x=POSXDRAW, p),
		...
		while ( newpos.x < termpos.x )
		{
			...
			DecorMixMx(&pmdecor, ppmicon,
					   termpos.x-oldpos.x,
					   newpos.x-oldpos.x);		/* adapte le decor "<" */
		}
		...
		while ( newpos.x > termpos.x )
		{
			newpos.x -= STEPX;
			if ( newpos.x < termpos.x )  newpos.x = termpos.x;
showmap (&pmdecor, "pmdecor") ;
showmap (ppmicon, "pmicon") ;
			DecorMixPx(&pmdecor, ppmicon,
					   oldpos.x-termpos.x,
					   oldpos.x-newpos.x);		/* adapte le decor ">" */
		}
		...
		while ( newpos.y < termpos.y )
		{
			...
			DecorMixMy(&pmdecor, ppmicon,
					   termpos.y-oldpos.y,
					   newpos.y-oldpos.y);		/* adapte le decor "^" */
		}
		...
		while ( newpos.y > termpos.y )
		{
			...
			DecorMixPy(&pmdecor, ppmicon,
					   oldpos.y-termpos.y,
					   oldpos.y-newpos.y);		/* adapte le decor "v" */
		}
		RestorePixmap(&pmdecor);				/* restitue le nouveau decor */
...
void DecorMake (short bSuperCel)
{
	...
	DecorShift(oldpos, newpos, 0);	/* fabrique le decor */
...
short DecorNewMonde (Monde *pm)
{
	...
	lastpmouse.x = -1;
	updatescreen = 1;						/* il faut mettre l'ecran a jour */
...
short DecorPartieRead (long pos, char file)
{
	...
	DecorMake(1);							/* refabrique le decor */
	IconDrawAll();							/* redessine toute la fenetre */
