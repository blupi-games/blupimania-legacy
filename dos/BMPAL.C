...
#define MAXICONY	4							/* nb max d'icones dans la palette */
...
static short	ticon[MAXICONY][MAXICONX];		/* icones des boutons */
static short	trest[MAXICONY][MAXICONX];		/* nb disponible */
static short	tspal[MAXICONY];				/* etats des sous-palettes (0..MAXICONX-1) */
static short	press;							/* bouton presse (0..MAXICONY-1) */
static short	typepress;						/* 1 -> palette avec bouton presse */

static short	tediticon[MAXEDITY][MAXEDITX];	/* edition palette: icones */
static short	teditetat[MAXEDITY][MAXEDITX];	/* edition palette: etats */
static short	teditrest[MAXEDITY][MAXEDITX];	/* edition palette: restes */
...
typedef struct
{
	short	typepress;						/* 1 -> palette avec bouton presse */
...
/*
	Donne le rang du bouton clique par la souris.
...
/*
	Dessine un bouton.
	state = 0	->	bouton relache normal
	state = 1	->	bouton presse
	state = 2	->	fleche ">" pour sous-palette
	state = 3	->	bouton relache pour action joueur
	state = 4	->	bouton presse pour action joueur
	state = 5	->	fleche ">" pour sous-palette action joueur
 */
...
void DrawButton (Pt pos, short icon, short state)
{
	...
	if ( state == 0 || state == 3 )
	{
		pos.x += 3;						/* decalage si bouton relache */
		pos.y += 7;
	}
	else
	{
		pos.x += 7;						/* decalage si bouton presse */
		pos.y += 2;
	}
...
/*
	Affiche le nb restant a cote d'un bouton.
 */
...
void DrawReste (Pt pos, short rest)
{
	...
	{
		pos.x += 4;
		chaine[0] = rest+'0';			/* unites */
		chaine[1] = 0;
	}
	else
	{
		chaine[0] = rest/10+'0';		/* dizaines */
		chaine[1] = rest%10+'0';		/* unites */
		chaine[2] = 0;
	}
...
/*
	Dessine le petit triangle en bas a cote de l'icone, pour indiquer
	la presence d'une sous-palette.
 */
...
void PaletteDraw (void)
{
	...
	for ( rang=0 ; rang<MAXICONY ; rang++ )
	{
		...
		{
			state = 0;
			if ( rang == press  )  state = 1;	/* bouton enfonce */
			if ( typepress == 0 )  state = 3;	/* bouton pour le joueur */
...
/*
	Donne le sous-rang du bouton clique par la souris.
...
short SPaletteOpen (short rang, Pixmap *ppm)
{
	...
	CopyPixel(0, pos, ppm, (p.y=0, p.x=0, p), dim, MODELOAD);	/* sauve l'ecran */
...
void SPaletteClose (short rang, Pixmap *ppm)
{
	...
	CopyPixel(ppm, (p.y=0, p.x=0, p), 0, pos, dim, MODELOAD);	/* restitue l'ecran */
	GivePixmap(ppm);
}
...
/*
	...
	Retourne 1 si le bouton a ete relache.
	Retourne 0 si la souris s'est deplacee vers la gauche.
 */
...
short SPaletteTracking (short rang)
{
	...
	DrawButton(GetButtonPos(rang), 1, type);		/* met la fleche -> */
...
/*
	Supprime un bouton dont le compteur d'utilisation est arrive a zero.
 */
...
/*
	Decremente le compteur d'utilisation d'un objet.
	Si le bouton correspondant a l'objet etait enfonce et que le compteur
	arrive a zero, enfonce un autre bouton automatiquement.
 */
...
void PaletteUseObj (short icon)
{
	...
	/*	Si le bouton ne peut plus etre utilise et qu'il etait visible
		et presse, cherche un autre en remplacement. */
	...
	DrawTriangle(rang);								/* enleve ev. le petit triangle */
...
/*
	Donne l'etat d'un bouton de la palette.
 */
...
/*
	Donne l'icone pressee dans la palette.
 */
...
/*
	Retourne un bouton special clique.
 */
...
short SpecButton (Pt pos)
{
	...
	static short table0[] =				/* table si jeu avec fleches */
	{
		25,92,17,17,	KEYUP,
		7,75,17,17,		KEYLEFT,
	...
	static short table1[] =				/* table si jeu avec telecommande */
	{
...
/*
	Ventilation d'un evenement pour la palette.
	Retourne un evenement (KEY*) lors d'une action directe.
	Retourne 1 si l'evenement n'etait pas pour la palette.
...
short PaletteEvent (short event, Pt pos)
{
	...
	if ( typepress )
	{
		typep = 1;		/* bouton presse */
		typer = 0;		/* bouton relache */
	}
	else
	{
		typep = 4;		/* bouton presse */
		typer = 3;		/* bouton relache */
	}
...
/*
	Initialise et dessine une nouvelle palette d'icones.
 */
...
/* Gestion de l'edition d'une palette (PHASE_PARAM) */
/* ------------------------------------------------ */
...
void DrawEditRest (Pt pos, short reste, short etat)
{
	...
	DrawFillRect(0, rect, MODELOAD, COLORBLANC);	/* efface l'indicateur precedent */
	
	if ( etat == 0 )  return;						/* si bouton relache -> pas d'indicateur */
	
	p.x = pos.x + LXICO/2 + 3;
	p.y = pos.y + TEXTSIZELIT + 2;
	chaine[0] = 125;
	chaine[1] = 0;
	DrawText(0, p, chaine, TEXTSIZELIT, MODELOAD);	/* dessine la fleche ^ */
	
	p.x = pos.x + LXICO/2 + 3;
	p.y = pos.y + LYICO/2 - 2;
	chaine[0] = 126;
	chaine[1] = 0;
	DrawText(0, p, chaine, TEXTSIZELIT, MODELOAD);	/* dessine la fleche v */
...
/*
	Donne les rangs (rx;ry) du bouton clique par la souris.
	Retourne 0 (false) si la souris n'est pas sur un bouton.
	Le type vaut 0 si icone ou +/-1 si fleche.
 */
		...
		for ( x=0 ; x<MAXEDITX ; x++ )
		{
				 ...
				 pos.y >= pb.y && pos.y <= pb.y+LYICO/2 )
			{
				...
				*py = y;
				...
				{
					*ptype = 0;			/* dans l'icone */
				}
				else
				{
					if ( teditetat[y][x] == 0 )  return 0;
					if ( pos.y < pb.y+LYICO/4 )  *ptype = +1;	/* dans la fleche superieure */
					else                         *ptype = -1;	/* dans la fleche inferieure */
				}
				return 1;				/* trouve */
			}
		}
...
/*
	Cherche les rangs (ry;rx) d'une icone.
	Si l'icone n'est pas trouvee, retourne 0 (false).
 */
...
/*
	Dessine la palette d'icones a editer.
 */
...
void PaletteEditOpen (short palette[])
{
	...
	/*	Met dans les tables tedit*[] les icones de tabpalette dans
		l'ordre standard. */
	...
	for ( y=0 ; y<MAXEDITY ; y++ )
	{
		for ( x=0 ; x<MAXEDITX ; x++ )
		{
			...
			teditetat[y][x] = 0;				/* bouton relache */
			teditrest[y][x] = 0;
		}
		if ( *pdesc == -1 )  pdesc++;
	}
	...
	while ( palette[i] != 0 )
	{
		...
		{
			teditetat[y][x] = 1;				/* bouton presse */
			teditrest[y][x] = palette[i+1];
		}
		i += 2;
	}
	
	/*	Affiche toutes les icones. */
...
/*
	Gestion d'un evenement pour editer la palette d'icones.
	Retourne 0 si l'evenement a ete traite.
 */
...
short PaletteEditEvent (short palette[], short event, Pt pos)
{
	...
	if ( type == 0 )							/* clic dans l'icone ? */
	{
		PlaySound(SOUND_CLIC);
		
		teditetat[y][x] ^= 1;					/* inverse l'etat du bouton */
...
/*
	Fin de l'edition d'une palette d'icones.
	Met a jour la palette de l'utilisateur.
 */
	...
	for ( y=0 ; y<MAXEDITY ; y++ )
	{
		...
		for ( x=0 ; x<MAXEDITX ; x++ )
		{
			if ( tediticon[y][x] != 0 && teditetat[y][x] == 1 )
			{
				...
				if ( total <= MAXICONY )
				{
					palette[i++] = -1;			/* passe toujours a la ligne si peu d'icones */
				}
				else
				{
					nb ++;
		...
		if ( nb > 0 )							/* au moins une icone dans cette ligne ? */
		{
			palette[i++] = -1;					/* passe a la ligne suivante */
		}
	}
	palette[i] = 0;
}
...
void InfoDraw (short status, short force, short vision, short mechant, short magic, short cles)
{
	...
	DuplPixel(&pm, &pminfo);			/* copie l'icone dans pminfo pour pouvoir modifier */
	...
	if ( force <= 30 )
	{
		rect.p1.x = 61;
		rect.p2.x = 61+4;
		DrawFillRect(&pminfo, rect, MODELOAD, COLORBLANC);	/* efface la lumiere overflow */
	}
	...
	if ( cles == 0 )
	{
		rect.p1.x = 1;
		rect.p2.x = 54;
		DrawFillRect(&pminfo, rect, MODELOAD, COLORBLANC);	/* efface la cle + ABC */
	}
	else
	{
		if ( (cles & (1<<0)) == 0 )
		{
			rect.p1.x = 28;
			rect.p2.x = 38;
			DrawFillRect(&pminfo, rect, MODELOAD, COLORBLANC);	/* efface la cle A */
		}
		if ( (cles & (1<<1)) == 0 )
		{
			rect.p1.x = 38;
			rect.p2.x = 46;
			DrawFillRect(&pminfo, rect, MODELOAD, COLORBLANC);	/* efface la cle B */
		}
		if ( (cles & (1<<2)) == 0 )
		{
			rect.p1.x = 46;
			rect.p2.x = 54;
			DrawFillRect(&pminfo, rect, MODELOAD, COLORBLANC);	/* efface la cle C */
		}
	}
...
short PalPartieRead (long pos, char file)
{
	...
	press = 0;
	PaletteDraw();					/* redessine la palette d'icones */
