...
#define MEMBW			820000		/* mÇmoire nÇcessaire pour noir-blanc */
#define MEMCOLOR		2048000		/* mÇmoire nÇcessaire pour couleur */
#define MEMRUNBW		273000		/* mÇmoire nÇcessaire pour variables */
#define MEMRUNCOLOR		385000		/* mÇmoire nÇcessaire pour variables */
#define MEMAUDIO		200000		/* mÇmoire nÇcessaire pour audio */
#define MEMBRUIT		390000		/* mÇmoire nÇcessaire pour BLUPIX01.AUDIO */
#define MEMMUSIC		290000		/* mÇmoire nÇcessaire pour BLUPIX02.AUDIO */

#define KSHIFT		(1<<5)			/* valeur Ö ajouter si SHIFT */
...
/* ----------------------------- */
/* Structure d'une image chargÇe */
/* ----------------------------- */

typedef struct
{
  Image		*head;			/* pointe l'en-tàte */
  char		*clut;			/* pointe la clut */
  char		*data;			/* pointe l'image codÇe */
}
ImagePC;
...
static long			atime = 0;					/* temps de dÇbut */

static u_long		nextrand[10];	     /* valeurs alÇatoires suivantes */

static short		colormode = 0;	     /* 1 = couleur possible */
static u_short		visumouse = 1;	     /* 1 = souris visible */
static Pt		lastmouse = {0,0};   /* derniäre position de la souris */
static KeyStatus	keystatus;	     /* Çtat des fläches du clavier */
static char		demo = 0;	     /* 1 = mode dÇmo */


static int havemouse ;	/* 1 => on a une souris, 0 => pas de souris */
static int lxwdo, lywdo ;  /* dimensions des fenàtres */
...
/*
	Retourne une valeur alÇatoire comprise entre min et max-1.
	min <= h < max
 */
...
void InitRandomEx (short g, short min, short max, char *pex)
{
  ...
  for( i=0 ; i<(max-min) ; i++ )
  {
    pex[i] = 0;			/* met tout le tableau Ö zÇro */
  }
}
...
/*
	Retourne une valeur alÇatoire exclusive.
 */
...
short GetRandomEx (short g, short min, short max, char *pex)
{
  ...
  for( i=0 ; i<(max-min) ; i++ )
  {
    if ( pex[val] == 0 )			/* valeur dÇjÖ sortie ? */
    {
      pex[val] = 1;				/* indique valeur utilisÇe */
      return min+val;
    }
    else
    {
...
/*
	Coup de sac du gÇnÇrateur alÇatoire.
		mode = 0	->	toujours la màme chose
		mode = 1	->	alÇatoire 100%
 */
...
/*
	Enclenche ou dÇclenche les bruitages.
 */
...
/*
	Test s'il est possible de donner un nouveau son Ö entendre.
...
/*
	Met en boucle (rÇpÇtition) les bruits donnÇs avec PlaySound.
	Si mode =  0  ->  normal (single)
	Si mode >  0  ->  nb de rÇpÇtitions
	Si mode = -1  ->  rÇpÇtition infinie (loop)
 */
...
/*
	Indique le dÇbut d'une opÇration plus ou moins longue
...
/*
	Indique la fin d'une opÇration plus on moins longue,
	avec attente si la durÇe Çtait plus petite que le temps total souhaitÇ.
 */
...
/*
	Donne la position de la souris, apräs avoir reáu
...
/*  rend des codes spÇciaux si les boutons de la
 *  souris ont ÇtÇ pressÇs ou relÉchÇs
 */
...
/*
	Lecture d'un ÇvÇnement du clavier, sans attendre.
	Retourne la touche pressÇe si != 0.
 */
...
/*
	Retourne l'Çtat du clavier, c'est-Ö-dire l'enfoncement
	Çventuel des touches fläches.
 */
...
/*
	DÇcale un pixmap dans l'une des quatre directions.
		*ppm		*pixmap source et destination
		shift.x		dÇcalage horizontal (<0 vers la droite, >0 vers la gauche)
		shift.y		dÇcalage vertical (<0 vers le bas, >0 vers le haut)
		color		couleur assignÇe Ö la nouvelle zone (0..15)
		*pzone		retourne la zone Ö redessiner apräs ScrollPixel
 */
...
/*
	...
	Cette procÇdure est la seule qui dessine dans l'Çcran !
		*ppms	*pixmap source (si == 0 -> Çcran)
		os		origine source (coin sup/gauche)
		*ppmd	*pixmap destination (si == 0 -> Çcran)
		od		origine destination (coin sup/gauche)
		dim		dimensions du rectangle
		mode	mode de transfert (MODELOAD/MODEOR/MODEAND)
	Retourne 1 si rien n'a ÇtÇ dessinÇ.
 */
...
#define STEPY 2
	  ...
	  if (0) //(os.x & 0x7) == 0 && (os.x & 0x7) == 0 && (dim.x & 0x7) == 0)
	  {
	  int lg = dim.x / 8 ;
	  char huge *ps = ppms->data + os.x / 8  + os.y * ppms->dxb;
	  char huge *pd = (char*)MK_FP (0xa000,0) + od.x / 8  + od.y * 80;
	  for (y = 0 ; y < dim.y ; y++)
	  {
		  oldptr = ps ;
		  select_plane(0);
...
/*
	PrÇpare le descripteur de fenàtre pgradesc pour pouvoir dessiner
	dans un pixmap en mÇmoire.
 */
...
/*
	Restitue le descripteur de fenàtre pgradesc pour pouvoir dessiner
	comme avant.
 */
...
/*
	Dessine un segment de droite d'un pixel d'Çpaisseur.
		*ppm		->	pixmap oó dessiner (0 = Çcran)
		p1			->	dÇpart
		p2			->	arrivÇe
		mode		->	mode de dessin
...
/*
	Dessine un rectangle d'un pixel d'Çpaisseur.
		*ppm		->	pixmap oó dessiner (0 = Çcran)
...
/*
	Dessine une surface rectangulaire remplie avec une couleur donnÇe
	dans un pixmap.
		*ppm		->	pixmap oó dessiner (0 = Çcran)
		rect.p1		->	coin sup/gauche
		rect.p2		->	coin inf/droite
...
TestHLine (Pixmap * ppm, short y)
{
  ...
  for (i = 0; i < LXICO / 8; i++)
    {
      if (*pt++ != (char)0xff) //@@ voir pour nb inversÇ
	return 0;
    }
  return 1;
}
...
TestVLine (Pixmap * ppm, short x)
{
  ...
  for (i = 0; i < LYICO; i++)
    {
      if (~(*pt) & mask)  // @@ voir pour nb inverÇ
	return 0;
      pt += ppm->dxb;
    }
  return 1;
...
/*
	Met une zone mÇmoire Ö zÇro (blanc) ou Ö un (noir).
 */
...
/*
	Charge un fichier image codÇ, si nÇcessaire.
 */
...
short GetPixmap(Pixmap *ppm, Pt dim, short fill, short color)
{
  ...
  if ( ppm->data != 0 )			/* pixmap existe dÇjÖ ? */
  {
    if ( ppm->dx == dim.x && ppm->dy == dim.y )
    {
      ClearMem(ppm->data, ((u_long)ppm->dxb * ppm->dy * nbp), fill);
      return 0;				/* pixmap existe dÇjÖ avec la bonne taille */
    }
    else
    {
      free(ppm->data);			/* libäre le pixmap prÇcÇdent */
      ppm->data = 0;
    }
  }
...
/*
	Libäre un pixmap quelconque, obtenu avec GetPixmap ou GetImage,
	mais pas avec GetIcon (give pas nÇcessaire).
 */
...
short GivePixmap(Pixmap *ppm)
{
  if ( ppm->data != 0 )
  {
    farfree(ppm->data);			/* libäre le pixmap */
    ppm->data = 0;
  }
  return 0;
}
...
/*
	Cache en mÇmoire une icìne en vue d'une utilisation prochaine.
	Ceci est utile pour le PC qui n'a pas assez de mÇmoire pour
	conserver toutes les icìnes !
 */
...
/*
	Conversion d'une icìne en pixmap noir/blanc.
	Le pointeur au data (ppm->data) est directement initialisÇ dans
	l'une des grandes images contenant toutes les icìnes (IMAICON
	ou IMAICON+IMAMASK).
	Puisque la mÇmoire pour le data n'est pas allouÇe par cette
	procÇdure, il ne faut donc pas faire de GivePixmap !
	Si le mode vaut zÇro, le data n'est pas rendu, mais seulement
...
//      XMSCopy (xmshandle, offset, 0, (long)(void *)buffer, 3200) ;
  ...
  {
    if (XMSErr == 0xa0)
      printf ("Pas assez de mÇmoire XMS. Consultez le fichier LISEZMOI.TXT\n") ;
    else
...
/*
	Chargement de l'image des icìnes.
 */
...
/*
	Efface tout l'Çcran (noir), pendant le changement de la clut.
 */
...
/*
		Lit nb bytes dans un fichier de donnÇes Ö la position pos.
...
/*
	Ecrit nb bytes dans un fichier de donnÇes Ö la position pos.
	Retourne 0 si l'Çcriture est ok.
 */
...
/*
	VÇrifie s'il reste assez de mÇmoire en fonction des options
...
/*
	Gestion des erreurs fatales de chargement Ö l'initialisation.
 */
...
static void FatalLoad(char *name, int err)
{
  ...
  if ( (int)errorname == 0 )
  {
    fprintf(stderr, "Erreur fatale numÇro %d avec %s ", err, name);
  }
  else
  {
...
/*
	Ouverture gÇnÇrale, chargement des librairies, gencar, etc.
 */
...
void OpenMachine(void)
{
  ...
  cprintf ("                          B L U P I M A N I A   1 . 3                           \n") ;
  ...
  err = LoadIcon();					/* charge l'image des icìnes */
  if ( err )
  {
    exit (0) ;
  }
  ...
  StartRandom(0, 0);					/* coup de sac du gÇnÇrateur alÇatoire toto */
  StartRandom(1, 1);					/* coup de sac du gÇnÇrateur alÇatoire dÇcor */
...
/*
	Retourne la longueur nÇcessaire pour sauver les variables de la partie en cours.
 */
