...
typedef struct
{
	short		idata;			/* permet de regenerer le pointeur pdata (*) */
	short		*pdata;			/* pointeur au data (fixe) */
	short		offset;			/* offset a partir de pdata */
	short		size;			/* taille du data (1..3) */
	short		loop;			/* nb de boucles a effectuer */
	short		index;			/* index */
	short		indexmax;		/* index maximal */
}
Auto;

/*	(*)	Chaque fois que pdata est initialise, idata l'est aussi, afin de permettre
		de regenerer pdata avec une procedure "Conv*ToTab*". Ceci est necessaire
		lorsque la structure de l'automate est sauvee puis reprise sur disque !
 */



/* --------------------------- */
/* Descripteur d'un toto anime */
/* --------------------------- */

#define OFFZTOTO	6		/* decalage en Z pour toto */
...
typedef struct
{
	Status		status;				/* etat general */
	short		joueur;				/* 1 si c'est le joueur */
	short		force;				/* force (60..0) */
	short		vitesse;			/* vitesse (-2..+2) */
	short		vision;				/* vision (0..1) */
	short		mechant;			/* mechant (0..1) */
	short		tank;				/* tank (0..4) */
	short		magic;				/* passe-muraille (100..0) */
	short		invincible;			/* invincible (100..0) */
	short		cles;				/* cles trouvees */
	Pt			poscel;				/* position cellule dans le monde */
	Pt			poscela;			/* position cellule d'arrivee */
	Pt			poscelb;			/* position cellule d'arrivee lointaine (si saute) */
	Action		action;				/* action en cours */
	Pt			lastpos;			/* derniere cellule atteinte */
	Pt			lastobjet;			/* coordonnees du dernier objet utilise */
	short		cntnotuse;			/* duree pour ne pas utiliser */
	short		lastrot;			/* dernier sens de rotation utilise */
	Pt			rstdecorcel;		/* cellule du decor a restaurer */
	short		rstdecoricon;		/* icone du decor a restaurer */
	short		nextrepos;			/* prochain repos possible */
	short		sequence;			/* numero de la sequence d'animation */
	short		energie;			/* energie d'une boule qui roule */
	
	Auto		autoicon;			/* automate pour les icones */
	Auto		automove;			/* automate pour les mouvements */
	short		moverang;			/* mouvement lineaire: rang */
	short		movemax;			/* mouvement lineaire: maximum */
	Pt			movelg;				/* mouvement lineaire: longueur */
	
	short		offz;				/* elevation verticale en z */
	Pt			posgra;				/* position graphique de l'icone */
	short		icon;				/* icone du moment */
}
TotoMove;
...
/* ------------------------------------- */
/* Descripteur d'un objet du decor anime */
/* ------------------------------------- */
...
typedef struct
{
	Pt			cel;			/* cellule ou est l'ascenseur */
	short		freq;			/* frequence des departs */
	short		count;			/* decompteur */
}
DepartMove;
...
static TotoMove		toto[MAXTOTO];				/* totos animes */
static TotoMove		back[MAXTOTO];				/* etats precedents */
static DepartMove	depart[MAXDEPART];			/* ascenseurs */
static ObjetMove	objet[MAXOBJET];			/* objets du decor animes */
static short		nbtoto;						/* nb de toto en mouvement simultanement */
static short		nbout;						/* nb de toto a sortir */
static short		nbin;						/* nb de toto a rentrer */
static short		perdu;						/* 1 -> c'est perdu */
static short		redraw;						/* 1 -> redessine tout */
static short		lastdetect;					/* dernier toto detecte pour les infos */
static short		lasttelecom;				/* derniere action de la telecommande */
static Pt			celcap1, celcap2;			/* cellule de cap pour le toto du joueur */
static short		vitessepousse;				/* vitesse a laquelle pousser une caisse */
static Action		caisseodir;					/* boule qui roule: direction */
static Action		caisseoddir;				/* boule qui roule: derniere direction */
static Pt			caisseocel;					/* boule qui roule: depart */
static short		gendecor;					/* numero de generation du decor */
static short		genstarttank;				/* numero de generation dernier depart tank */



typedef struct
{
	short		nbtoto;						/* nb de toto en mouvement simultanement */
	short		nbout;						/* nb de toto a sortir */
	short		nbin;						/* nb de toto a rentrer */
	short		lastdetect;					/* dernier toto detecte pour les infos */
	Pt			celcap1, celcap2;			/* cellule de cap pour le toto du joueur */
	Action		caisseodir;					/* boule qui roule: direction */
	Action		caisseoddir;				/* boule qui roule: derniere direction */
	Pt			caisseocel;					/* boule qui roule: depart */
...
short AutoNext (Auto *p, short result[])
{
	...
	if ( p->loop == 0 )				/* operation suivante ? */
	{
		c = p->pdata[p->offset];
...
/*
	Affiche un pixmap dans le coin sup/gauchde de l'ecran, pour le debug.
 */
...
/*
	Cette procedure est appeles chaque fois qu'une cellule du decor
	a ete modifiee.
 */
...
/*
	Retourne 1 si le toto est sur la cellule de depart.
	Retourne 2 si le toto est sur la cellule d'arrivee.
...
/*
	Determine l'obstacle eventuel place sur une cellule.
	Il peut s'agit d'un objet du decor ou d'un autre toto !
	Retourne 0 s'il n'y a pas d'obstacle.
		toto = 1	->	tient compte des toto
		toto = 0	->	ne tient compte que du decor
 */
...
short GetObstacle (Pt cel, short toto)
{
	...
	if ( i == -1             ||				/* sort du monde */
		 i >= ICO_BLOQUE     ||				/* obstacle du decor */
		 i == ICO_ARRIVEE    ||				/* arrivee */
		 i == ICO_DEPART     ||				/* depart */
		 i == ICO_TROU       ||				/* trou */
		 i == ICO_TROUBOUCHE ||				/* trou bouche */
...
/*
	Donne l'orientation correspondant a une action.
...
/*
	Donne le sens de la rotation correspondant a une action.
...
/*
	Initialise un toto anime sur une cellule, pret a partir avec une action.
	Retourne le rang du toto initialise (-1 si erreur).
 */
...
short MoveInit (Pt poscel, short action, short posz)
{
	...
	NewAction(i, action, posz);		/* demarre une action */
...
/*
	Fait eventuellement partir les ascenseurs.
 */
...
void DepartNext (void)
{
	...
	{
		if ( depart[d].cel.x < 0 )  continue;			/* continue si ascenseur inutilise */
		
		cel = depart[d].cel;
		cel.x ++;
		if ( GetObstacle(cel, 1) != 0 )  continue;		/* continue si cellule occupee */
		...
		if ( depart[d].count == 0 )
		{
			...
			MoveInit(depart[d].cel, AC_DEPART_E, 150);			/* .. avec toto accroche */
			nbout --;
			if ( nbout == 0 || nbtoto > MAXTOTO-2 )  return;
		}
	}
...
/*
	Bouge si necessaire un objet anime du decor.
 */
...
void ObjetNextOne (short i)
{
	...
	else
	{
		...
		if ( icon != 0 )
		{
			DecorModif(objet[i].cel, icon);		/* modifie le decor */
		}
	}
}
...
/*
	Bouge si necessaire les objets animes du decor.
 */
...
/*
	Met un objet anime dans la liste.
	...
	for ( i=0 ; i<MAXOBJET ; i++ )
	{
		if ( objet[i].status == STVIDE )
		{
			...
			if ( obj >= OB_SENSUNIO &&
				 obj <= OB_SENSUNIS )
			{
				ObjetNextOne(i);	/* modifie le decor immediatement */
			}
...
/*
	Genere un mouvement quelconque selon une table.
 */
...
/*
	Initialise un mouvement lineaire regulier, dans le plan (z=0).
		nbmove	->	nb de pas a effectuer
		lg		->	deplacement a effectuer (+/- dx;dy)
 */
...
/*
	Calcule un mouvement lineaire regulier, dans le plan (z=0).
	Retourne 1 (true) si le mouvement est termine.
 */
...
short CalcMovie(short i, short result[])
{
	...
	result[0] = new.x - last.x;		/* deplacement selon x */
	result[1] = new.y - last.y;		/* deplacement selon y */
	result[2] = 0;					/* deplacement selon z */
...
/*
	Donne le type de marche a utiliser pour un toto donne.
...
/*
	Demarre une nouvelle action pour un toto.
 */
...
void NewAction(short i, Action action, short posz)
{
	...
	if ( action >= AC_MARCHE_E &&				/* toto doit marcher ? */
		 action <= AC_MARCHE_S &&
		 ((toto[i].action >= AC_STOP_E &&		/* toto est stoppe ? */
		   toto[i].action <= AC_STOP_S) ||
		  (toto[i].action >= AC_VISION_E &&		/* toto a recu des lunettes ? */
		   toto[i].action <= AC_VISION_S) ||
		  (toto[i].action >= AC_REPOS_E &&		/* toto s'est repose ? */
		   toto[i].action <= AC_REPOS_S) ||
		  (toto[i].action >= AC_REFLEXION_E &&	/* toto s'est gratte la tete ? */
		   toto[i].action <= AC_REFLEXION_S) ||
		  (toto[i].action >= AC_YOYO_E &&		/* toto a joue au yoyo ? */
		   toto[i].action <= AC_YOYO_S) ||
		  (toto[i].action >= AC_HAUSSE_E &&		/* toto a hausse les epaules ? */
		 ...
		 nbmove != toto[i].movemax )
	{
		...
		AutoInit(&toto[i].autoicon, action, ptable, 1);		/* init les icones */
	}
	...
	switch( action )
	{
		...
		case AC_START_E:
			...
			toto[i].poscelb.x ++;						/* comme si cellule deja occupee ! */
			break;
		case AC_START_O:
			InitMovie(i, 2, (p.y=0, p.x=0, p));
			toto[i].poscelb.x --;						/* comme si cellule deja occupee ! */
		...
		case AC_START_S:
			InitMovie(i, 2, (p.y=0, p.x=0, p));
			toto[i].poscelb.y ++;						/* comme si cellule deja occupee ! */
			break;
		case AC_START_N:
			InitMovie(i, 2, (p.y=0, p.x=0, p));
			toto[i].poscelb.y --;						/* comme si cellule deja occupee ! */
...
/*
	Retourne l'obstacle present dans une cellule, comme GetObstacle.
...
/*
	...
	(mur du decor, autre toto, etc.).
	Essaye de le tourner dans le meme sens que le precedent utilise !
 */
...
/*
	Fait peter toutes les bombes correspondant a un detonateur.
 */
...
/*
	Regarde s'il est possible de sauter par dessus un objet du decor
	pas trop haut.
...
/*
	Verifie s'il est possible de pousser une caisse.
	Demarre eventuellement les actions associees.
...
short IfPousseCaisse (Pt celarr, short caisse, Action orientation)
{
		  ...
		  caisse == ICO_CAISSEO ) )
	{
		if ( icon == ICO_TROU )
		{
			DecorModif(celarr, ICO_TROUBOUCHE);		/* met un trou bouche */
		}
		if ( icon == ICO_SENSUNI_O && orientation == AC_MARCHE_E )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIO);		/* monte puis redescend la fleche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_SENSUNI_E && orientation == AC_MARCHE_O )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIE);		/* monte puis redescend la fleche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_SENSUNI_N && orientation == AC_MARCHE_S )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIN);		/* monte puis redescend la fleche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_SENSUNI_S && orientation == AC_MARCHE_N )
		{
			if ( MoveGetCel(celarr) == 0 )
				ObjetPut(celarr, OB_SENSUNIS);		/* monte puis redescend la fleche */
			return 0;								/* impossible */
		}
		if ( icon == ICO_VITRE+4 )
		{
...
/*
	Effectue une action speciale, en fonction de l'obstacle rencontre.
...
short SpecAction(short i, short obstacle, Pt testcel)
{
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_N )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIS);			/* monte puis redescend la fleche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la fleche */
		}
		return 1;
	}
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_S )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIN);			/* monte puis redescend la fleche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la fleche */
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_O )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIE);			/* monte puis redescend la fleche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la fleche */
		 ...
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_E )
		{
			if ( toto[i].joueur )  celcap1.x = -1;
			TourneAction(i);
			ObjetPut(testcel, OB_SENSUNIO);			/* monte puis redescend la fleche */
		}
		else
		{
			NewAction(i, orientation, 0);			/* passe sur la fleche */
	...
	if ( obstacle == ICO_ACCEL_S &&					/* accelerateur vers le sud ? */
		 MoveGetCel(testcel) == 0 )
	{
	...
	if ( obstacle == ICO_ACCEL_E &&					/* accelerateur vers l'est ? */
		 MoveGetCel(testcel) == 0 )
	{
	...
	if ( obstacle == ICO_ACCEL_N &&					/* accelerateur vers le nord ? */
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_N )  toto[i].vitesse = +2;
		if ( orientation == AC_MARCHE_S )  toto[i].vitesse = -2;
	...
	if ( obstacle == ICO_ACCEL_O &&					/* accelerateur vers l'ouest ? */
		 MoveGetCel(testcel) == 0 )
	{
		if ( orientation == AC_MARCHE_O )  toto[i].vitesse = +2;
		if ( orientation == AC_MARCHE_E )  toto[i].vitesse = -2;
	...
	if ( obstacle == ICO_ARRIVEE )					/* est-on sur l'arrivee ? */
	{
		if ( toto[i].joueur )  celcap1.x = -1;
		if ( toto[i].mechant == 0 )
		{
	...
	if ( obstacle == ICO_AIMANT )					/* arrive a un aimant ? */
	{
		DecorModif(testcel, DecorGetInitCel(testcel));	/* enleve l'aimant du decor */
		 ...
		 toto[i].vision == 0 )
	{
		...
		DecorModif(testcel, DecorGetInitCel(testcel));	/* enleve les lunettes du decor */
		
		NewAction(i, orientation+AC_VISION_E-AC_MARCHE_E, 0);
		return 1;
	}
	
	if ( (obstacle == ICO_TABLEBOIT ||				/* arrive a une table ? */
		  obstacle == ICO_TABLEPOISON) &&			/* arrive a une table ? */
		 toto[i].mechant == 0 &&
		  ...
		  toto[i].lastobjet.y != testcel.y ) )
	{
		...
		DecorModif(testcel, ICO_TABLEVIDE);			/* enleve la bouteille sur la table */
	...
	if ( obstacle == ICO_TOURTE &&					/* arrive a une tourte ? */
		 toto[i].mechant == 0 )
	{
		if ( toto[i].joueur )  celcap1.x = -1;

		DecorModif(testcel, ICO_TOURTE+1);			/* enleve la tourte sur la table */
	...
	if ( obstacle == ICO_MAGIC &&					/* arrive a la magie ? */
		 toto[i].mechant == 0 )
	{
		if ( toto[i].joueur )  celcap1.x = -1;
	...
	if ( obstacle == ICO_LIVRE &&					/* arrive a un livre ? */
		 toto[i].vision  == 1  &&
		 toto[i].mechant == 0  &&
		 (toto[i].lastobjet.x != testcel.x ||
		  toto[i].lastobjet.y != testcel.y ) )
	{
		if ( toto[i].joueur )  celcap1.x = -1;
		DecorModif(testcel, DecorGetInitCel(testcel));	/* enleve le livre */
	...
	if ( ((obstacle >= ICO_MEUBLE+0 &&				/* arrive a une commode ? */
	...
	if ( obstacle >= ICO_CLE_A &&					/* arrive a une cle ? */
		 obstacle <= ICO_CLE_C &&
		 ...
		 toto[i].mechant == 0 )
	{
		...
		DecorModif(testcel, ICO_CLE_VIDE);			/* decroche la cle */
		NewAction(i, orientation+AC_CLE_E-AC_MARCHE_E, 0);
		return 1;
	}
	
	if ( obstacle >= ICO_PORTEF_EO   &&				/* arrive a une porte est-ouest fermee ? */
	...
	if ( obstacle >= ICO_PORTEF_NS   &&				/* arrive a une porte nord-sud fermee ? */
	...
	if ( obstacle == ICO_BAISSE &&					/* arrive a une porte electronique ouest ? */
	...
	if ( obstacle == ICO_GLISSE )					/* glisse sur une peau de banane ? */
	{
		...
		if ( toto[i].joueur )  celcap1.x = -1;
		toto[i].mechant = 1;						/* devient mechant */
	...
	if ( (obstacle == ICO_CAISSE  ||				/* arrive derriere une caisse ? */
		 ...
		 toto[i].vision  == 1 )
	{
		if ( toto[i].force > 5 )					/* assez de force pour pousser ? */
		{
			...
			if ( IfPousseCaisse(cel, obstacle, orientation) )
			{
				...
				if ( cel2.x != -1 )								/* 2 caisses a pousser ? */
				{
					DecorModif(cel2, DecorGetInitCel(cel2));	/* enleve la caisse fixe */
				...
				DecorModif(testcel, DecorGetInitCel(testcel));	/* enleve la caisse fixe */
				if ( obstacle == ICO_CAISSE )
				{
				...
				if ( rang >= 0 )
				{
					...
					if ( obstacle == ICO_CAISSEO ||
						 obstacle == ICO_CAISSEG )
					{
						toto[rang].energie = 2;		/* energie d'une boule poussee */
					}
					redraw = 1;						/* faudra tout redessiner */
				}
				return 1;
		  ...
		  toto[i].lastobjet.y != testcel.y ) )
	{
		...
		NewAction(i, AC_ELECTRO_O, 0);				/* toto s'electrocute */
		return 1;
	}
	
	if ( obstacle == ICO_TELE &&					/* est-on devant une television ? */
		  ...
		  toto[i].lastobjet.y != testcel.y ) )
	{
		...
		NewAction(i, AC_TELE_N, 0);					/* toto regarde la tele */
		ObjetPut(testcel, OB_TELE);					/* allume la tele */
		return 1;
	}
...
/*
	...
	Si back = 1, cela signifie qu'on teste une cellule situee derriere
...
short IfSuperAction (short i, Pt cel, Action direction, short back)
{
	...
	/*	Si l'objet place dans cette cellule est le dernier utilise
		par ce toto, ne le considere pas comme chouette. */
	...
	if ( toto[i].tank != 0 )
	{
		...
		return 0;								/* ne s'interesse a rien d'autre si tank */
	}
	...
	if ( toto[i].vision == 0 ||					/* ne s'interesse a rien d'autre si aveugle */
		 back )                   return 0;		/* ne s'interesse a rien d'autre si dans le dos */
	
	if ( icon == ICO_ARRIVEE )    return 1;		/* arrivee */
	
	if ( toto[i].mechant == 1 &&
		 icon >= ICO_DETONATEUR_A &&
		 icon <= ICO_DETONATEUR_C )  return 1;
	
	if ( toto[i].mechant == 1 )   return 0;		/* ne s'interesse a rien d'autre si mechant */
	...
	if ( icon == ICO_BAISSE &&					/* porte electronique */
...
/*
	Regarde s'il y a une cellule interessante dans une direction donnee.
	Si oui, retourne 1 (true).
 */
...
short IfLineAction (short i, Pt cel, Action direction, short back)
{
	...
	for ( nb=0 ; nb<10 ; nb++ )
	{
		...
		/* retourne true si objet interessant */
			 ...
			 toto[i].tank    == 0 )
		{
				  ...
				  icon == ICO_LUNETTES )			/* lunettes ? */
			{
				...
				if ( obstacle == 0 ||				/* libre apres l'obstacle bas ? */
					 obstacle == ICO_AIMANT )
				{
					pos.x = cel.x - dir.x;
					pos.y = cel.y - dir.y;
		...
		if ( icon == ICO_GLISSE ||					/* peau de banane ? */
			 ...
			 (icon >= ICO_ACCEL_S &&
			  icon <= ICO_ACCEL_O) )				/* le regard passe par-dessus un accelerateur */
		{
			continue;
		}
...
/*
	Teste si toto depasse un mur en coin, et s'il faut eventuellement
	tourner plutot que de continuer toujours tout droit.
...
short TurnCoin (short i, Action *pnextaction)
{
	...
	if ( GetMur(bleft, 0) >= ICO_BLOQUE &&				/* mur a l'arriere gauche ? */
		 GetMur(left, 1) == 0 &&						/* passage a gauche ? */
		 (left.x != toto[i].lastpos.x ||
		  left.y != toto[i].lastpos.y ) )				/* ne vient-on pas deja de la ? */
	{
		*pnextaction = acleft;
		return 1;
	}
	
	if ( GetMur(bright, 0) >= ICO_BLOQUE &&				/* mur a l'arriere droite ? */
		 GetMur(right, 1) == 0 &&						/* passage a droite ? */
		 (right.x != toto[i].lastpos.x ||
		  right.y != toto[i].lastpos.y ) )				/* ne vient-on pas deja de la ? */
	{
		*pnextaction = acright;
		return 1;
	}
...
/*
	Si toto est dote de la vision, regarde s'il n'y a pas mieux a faire
	que de marcher tout droit.
	Retourne 1 (true) si y'a qq chose de mieux a faire.
 */
...
short VisionAction (short i, Action *pnextaction)
{
	...
	/*	Si toto s'est deja fixe un but, ne change pas d'avis ! */
...
/*
	Fait eventuellement entendre un son d'ambiance selon
	un obstacle rencontre.
 */
...
/*
	Cherche une action pour le toto pilote par le joueur.
 */
...
void JoueurAction (short i, char event, Action orientation, Pt testcel)
{
	...
	/*	Deplace le toto du joueur selon les actions de la telecommande. */
	
	eventcont = event;
	if ( event == 0 && lasttelecom != 0 )
	{
		eventcont = lasttelecom;			/* continue si bouton toujours presse */
	}
	...
	if ( event == KEYGOLEFT )				/* tourne a gauche */
	{
		celcap1.x = -1;
		if ( orientation == AC_MARCHE_E )
		{
	...
	if ( event == KEYGORIGHT )				/* tourne a droite */
	{
		celcap1.x = -1;
		if ( orientation == AC_MARCHE_E )
		{
	...
	/*	Deplace le toto du joueur selon les touches fleches. */
	
	rekey:
	if ( keystatus == STRIGHT )				/* va a droite */
	{
	...
	if ( keystatus == STLEFT )				/* va a gauche */
	{
		if ( retry == 0 )  celcap1.x = -1;
		if ( orientation == AC_MARCHE_O )
		{
	...
	/*	Deplace le toto du joueur selon le cap choisi. */
	
	if ( celcap1.x == toto[i].poscel.x &&
		 celcap1.y == toto[i].poscel.y &&
		 celcap2.x != -1 )
	{
		celcap1 = celcap2;					/* va sur le deuxieme cap */
		celcap2.x = -1;
	}
	...
	/*	Si y'a vraiment rien d'autre a faire, stoppe. */
...
/*
	Verifie si le trajet sur une ligne droite est libre d'obstacles.
	Si oui, retourne 1 (true).
	Ne teste pas la cellule d'arrivee !
 */
...
/*
	Retourne le numero du toto du joueur.
...
/*
	Cherche un nouveau cap a atteindre pour le toto du joueur en fonction
	de la cellule cliquee dans le decor.
 */
...
void JoueurCap (char event, Pt pmouse)
{
	...
	cel = DecorDetCel(pmouse);			/* cherche la cellule cliquee */
	...
	inter.y = cel.y;
	if ( IfFreeLine(toto[i].poscel, inter) &&	/* accessible en "L" inverse ? */
		 IfFreeLine(inter, cel) )
	{
		celcap1 = inter;
		celcap2 = cel;
...
/*
	...
	Cette procedure est appelee chaque fois qu'une action quelconque
	est terminee, pour determiner ce que toto fera ensuite.
 */
...
void NextAction(char event, short i)
{
	...
	if ( toto[i].action >= AC_SAUTE1_E &&	/* saut commence ? */
	...
	if ( toto[i].action >= AC_START_E &&	/* marche commencee ? */
	...
	back[i] = toto[i];					/* conserve l'etat pour pouvoir faire un pas en arriere */
	...
	{
		p = toto[i].lastobjet;
		DecorModif(toto[i].rstdecorcel, toto[i].rstdecoricon);	/* restaure le decor */
		toto[i].rstdecoricon = 0;
		toto[i].lastobjet = p;
	}
	...
	{
		NewAction(i, AC_EXPLOSE_E + toto[i].action-AC_BOIT_E, 0);	/* toto a trop mange ! */
		toto[i].force = 0;
	...
	if ( toto[i].joueur )
	{
		...
		SoundAmbiance(obstacle);				/* ev. son d'ambiance */
	}
		 ...
		 toto[i].action <= AC_ARRIVEE_S )
	{
		...
		MoveInit(testcel, AC_BALLON_M, -LYICO);		/* depart du ballon */
		DecorModif(testcel, ICO_ARRIVEEVIDE);		/* detache le ballon */
		return;
	}
		 ...
		 toto[i].action == AC_TANK )
	{
		...
		if ( toto[i].tank == 3 )
		{
			DecorModif(toto[i].poscel, ICO_TANKBAS);		/* met un sol bouche par le tank */
		 ...
		 toto[i].action <= AC_CAISSEOD_S )
	{
		...
		{
			if ( toto[i].energie == 2 )					/* energie d'une boule poussee ? */
			{
	...
	{
		DecorModif(toto[i].poscel, ICO_CAISSEBAS);		/* met un sol bouche par la caisse */
	...
	{
		DecorModif(toto[i].poscel, ICO_CAISSEVBAS);		/* met un sol bouche par la caisse */
	...
	{
		DecorModif(toto[i].poscel, ICO_CAISSEOBAS);		/* met un sol bouche par la caisse */
	...
	{
		DecorModif(toto[i].poscel, ICO_CAISSEGBAS);		/* met un sol bouche par la caisse */
	...
	{
		NewAction(i, nextaction, 0);		/* oui -> demarre la nouvelle action */
		return;
	}
	
	obstacle = GetObstacle(testcel, 1);
	if ( obstacle == 0 )					/* mouvement projete possible ? */
	{
		NewAction(i, nextaction, 0);		/* si c'est possible -> demarre la nouvelle action */
	}
	else
	{
		if ( !SpecAction(i, obstacle, testcel) )	/* effectue une action speciale */
		{
			if ( toto[i].mechant == 0 &&
				 toto[i].tank == 0 )
			{
			...
			TourneAction(i);						/* tourne si y'a rien d'autre a faire */
		}
	}
}
...
/*
	Fait faire un pas en arriere au toto place sur une cellule donnee.
 */
...
void MoveBack (Pt cel)
{
	...
	for ( i=0 ; i<MAXTOTO ; i++ )
	{
			 ...
			 (toto[i].poscelb.x == cel.x && toto[i].poscelb.y == cel.y) )
		{
			...
			{
				toto[i] = back[i];		/* reprend l'etat precedent */
			}
		}
	}
}
...
/*
	Genere une table triee contenant les indices dans toto[] avec
	en premier les totos places les plus au fond, et en dernier ceux
	qui sont tout devant. Ceci est necessaire, pour dessiner avec
	IconDrawPut a partir du fond jusque devant.
	Trie avec un "bubble-sort" pas fameux du tout, mais cela ne gene
	pas trop, vu le petit nombre de toto a trier (<10) !
 */
...
void TrieToto (char *ptable)
{
	...
	for ( i=0 ; i<MAXTOTO ; i++ )
	{
		ptable[i] = i;			/* genere une table par defaut */
	}
	...
	while ( job != 0 );		/* repere tant que qq a ete fait */
}
...
/*
	Detecte le toto vise par la souris.
...
short DetectToto (Pt pmouse, Pt ovisu, char ordre[])
{
	...
	{
		for ( j=MAXTOTO-1 ; j>=0 ; j-- )	/* du plus en avant au plus en arriere */
		{
			i = ordre[j];
			...
			{
				lastdetect = i;
				return i;					/* retourne le toto trouve */
			}
		}
	}
	...
	for ( i=0 ; i<MAXTOTO ; i++ )
	{
		...
		{
			lastdetect = i;
			return i;						/* retourne le premier toto trouve dans les tables */
		}
	}
	
	return -1;								/* rien trouve */
}
...
/*
	...
	est interruptible, suite a un evenement prioritaire.
 */
...
void DrawOne (short i, Pt ovisu)
{
	...
	Reg			r;
	Pt			pos;
	Pt			celdd;						/* cellule charniere pour devant/derriere */
...
/*
	Fait eventuellement partir les tanks stoppes si y'a un toto qui s'pointe.
 */
...
void StartTank (void)
{
	...
	if ( typeedit != 0 )  return;			/* pas de depart si on est en train d'editer */
	
	if ( genstarttank == gendecor )  return;	/* rien a faire si toujours le meme decor */
	...
	for ( cel.y=0 ; cel.y<=MAXCELY ; cel.y++ )
	{
		for ( cel.x=0 ; cel.x<=MAXCELX ; cel.x++ )
		{
			...
			if ( icon == ICO_TANK_X  ||		/* est-ce un tank immobile (dans le decor) ? */
				 icon == ICO_TANK_EO ||
				 icon == ICO_TANK_NS )
			{
				i = MoveInit(cel, AC_STOP_E, 0);					/* prepare le tank */
				if ( i >= 0 )
				{
...
/*
	Avance les totos animes d'un "pas".
	Retourne 1 si le monde est termine (tous les toto rentres).
	Retourne 2 si le monde est termine (perdu).
 */
...
short MoveNext (char event, Pt pmouse)
{
	...
	if ( event == KEYCLICREL )
	{
		lasttelecom = 0;					/* stoppe si bouton souris relache */
	}
	
	if ( (event == KEYCLIC || event == KEYCLICR) && typejeu == 1 )
	{
		JoueurCap(event, pmouse);			/* assigne ev. un nouveau cap a atteindre */
	}
	
	DepartNext();							/* gere les ascenseurs */
	ObjetNext();							/* gere les objets du decor */
	
	TrieToto(ordre);						/* genere la table ordre[] */
	
	ovisu = DecorGetOrigine();
	
	i = DetectToto(pmouse, ovisu, ordre);	/* detecte le toto vise par la souris */
	if ( i < 0 )
	{
	...
	StartTank();							/* fait ev. partir les tanks stoppes */
	...
	for ( j=0 ; j<MAXTOTO ; j++ )
	{
		...
		if ( CalcMovie(i, result) ||				/* action terminee ? */
			 (try < 1 && IfInterrupt(i, event)) )
		{
			NextAction(event, i);					/* oui -> affecte une autre action a toto */
			try ++;
			goto next;
		}
		...
		{
			toto[i].invincible --;					/* l'invincibilite diminue un peu */
		}
	
		AutoNext(&toto[i].autoicon, result);		/* cherche l'icone suivante */
		if ( toto[i].mechant == 0 ||
		...
		{
			toto[i].icon += 80;				/* icone de toto aveugle */
		}
		...
		if ( toto[i].force < 5 && (
			 ...
			 (toto[i].icon >= 16 && toto[i].icon <= 19) ))
		{
			toto[i].icon += 32;				/* icone de mauvaise humeur */
		}
		
		if ( toto[i].mechant == 1 &&
			 (toto[i].icon >= 1  && toto[i].icon <= 12) )
		{
			toto[i].icon += 112;			/* icone mechant */
		}
		...
		{
			toto[i].icon += 128;			/* icone avec antenne */
		}
	...
	if ( caisseoddir != 0 )
	{
		...
		{
			DecorModif(caisseocel, DecorGetInitCel(caisseocel));	/* enleve la boule fixe */
			i = MoveInit(caisseocel, caisseoddir+AC_CAISSEOD_E-AC_CAISSEO_E, 0);
			if ( i >= 0 )
			{
				toto[i].icon = ICO_CAISSEO;
				toto[i].energie = 1;			/* energie d'une boule sur son elan */
				redraw = 1;						/* faudra tout redessiner */
			}
		}
	}
	...
	if ( nbin <= 0 )  return 1;				/* tous les toto sont rentres */
	return 0;
}
...
/*
	Construit devant le toto du joueur, selon l'outil actionne.
...
short MoveBuild (short outil)
{
	...
	temp = typejeu;
	typejeu = 0;							/* comme si jeu sans toto telecommande */
	err = DecorEvent(pos, 1, outil);		/* modifie le decor */
	typejeu = temp;
...
/*
	Decale le decor si le toto du joueur depasse de la partie visible.
 */
...
void MoveScroll (short quick)
{
	...
	if ( typeedit == 0 && i < 0 && typejeu == 1 )
	{
		...
		typejeu = 0;									/* jeu sans toto telecommande */
	}
...
void MoveNewMonde (short freq)
{
	...
	StartRandom(0,0);					/* aleatoire repetitif pour les toto */
	StartRandom(1,1);					/* aleatoire total pour les decors */
	...
	for ( cel.y=0 ; cel.y<=MAXCELY ; cel.y++ )
	{
		for ( cel.x=0 ; cel.x<=MAXCELX ; cel.x++ )
		{
			...
			if ( icon == ICO_ARRIVEE )		/* est-ce une arrivee (ballon) ? */
			{
				nbout ++;
			}
			if ( icon == ICO_JOUEUR &&		/* est-ce le toto du joueur ? */
				 typeedit == 0 )			/* n'est-on pas en train d'editer un monde ? */
			{
				typejeu = 1;
				DecorPutCel(cel, DecorGetInitCel(cel));		/* remet un sol normal */
				if ( nbjoueurs == 0 )
				{
					i = MoveInit(cel, AC_STOP_E, 0);		/* prepare le toto du joueur */
					if ( i >= 0 )
					{
			...
			if ( icon >= ICO_TANK_E &&		/* est-ce un tank ? */
				 ...
				 typeedit == 0 )			/* n'est-on pas en train d'editer un monde ? */
			{
				i = MoveInit(cel, icon-ICO_TANK_E+AC_STOP_E, 0);	/* prepare le tank */
				if ( i >= 0 )
				{
					DecorPutCel(cel, DecorGetInitCel(cel));			/* remet un sol normal */
	...
	if ( typeedit || nbout == 0 )			/* edition d'un monde en cours ? */
	{
...
/*
	...
	Les pointeurs contenus dans les automates (pdata) doivent etre
	regeneres en fonction de l'action (idata) !
 */
