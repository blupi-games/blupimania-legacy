...
#endif
  int key;
  static int last_returned = 0;

  *ppos = getmousepos ();	/* lit la position de la souris */

#ifdef DEMOVERS
  if (ppos->x != lastmpos.x)
    clr_gametimeout () ;
  lastmpos.x = ppos->x ;
#endif
  ...
  if (_BX != mousebut)
      ...
      else
	{
	  ...
	  switch (key)
	    ...
	    case KEY_Ctrl_p:
	      hidemouse ();
	      /*	printscreen(0,0,640,350);   */
	      showmouse ();
	      return -1;
	    case KEY_Ctrl_d:
	      if (demo_onoff)
		{
		  sound_onoff = 1;	/* sound on */
		  demo_onoff = 0;	/* demo off */
		}
	      else
		{
		  sound_onoff = 0;	/* sound off */
		  demo_onoff = 1;	/* demo on */
		}
	      return -1;
//	    case KEY_F10:
//	      debug_mem () ;
...
//      tcol_loadclut(pim->clut); /* change la clut */
  ...
  if (numero == 0 || numero >= 300)
    sprintf(name, "BP%03d.IMA", numero) ;
  else
    sprintf(name, "BP%03d.COL", numero);
...
/* utilise la clut de la derniŠre image charg‚e */
...
/* donne le pointeur … la clut de la derniŠre image charg‚e */

void getclut (char **pclut, int *lgclut)
{
  *pclut = pim->clut ;
  *lgclut = pim->head->imlgc ;
}
...
//  signal (SIGFPE, catcher) ;
  ...
  if (farcoreleft() < 434000L)
  {
    void errmem (long, long) ;
    errmem (84000L, 520000L) ;
  }
...
#if 0    /* ici on n'a pas d'image de bienvenue */
...
/* b&w screen */
  {
    ...
    {
      ifhidemouse(&src);  /* cache la souris */
      src.bytes=640/8;
    ...
    {
      ifhidemouse(&dest); /* cache la souris */
      dest.bytes=640/8;
