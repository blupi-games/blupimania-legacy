...
static Pixmap	pmchar1;			/* pixmap des caractäres */
static Pixmap	pmchar2;			/* pixmap des caractäres */

static char		*pchaine;			/* pointe chaåne ÇditÇe */
static short	lgchaine;			/* longueur de la chaåne */
...
static char		ifcx;				/* 1 -> curseur "|" prÇsent */
static short	cx;					/* position de curseur "|" en x */
static short	begin;				/* dÇbut oó afficher [car] */
static short	charsize;			/* taille des caractäres */
...
/*
	Retourne la largeur d'un caractäre.
 */
...
/*
	Dessine un caractäre et avance la position pour le suivant.
 */
...
/*
	Dessine un caractäre accentuÇ ou autre.
 */
...
void DrawAccent (Pixmap *ppm, Pt *ppos, char c, ShowMode mode)
{
	...
	{
		if ( c == '\n' )  c = 127;					/* petit triangle ">" */
		DrawChar(ppm, ppos, c, mode);				/* dessine le caractäre */
	}
}
...
/*
	Dessine une chaåne de caractäres terminÇe par zÇro.
	Retourne la position suivante, sur la ligne de base.
		*ppm		->	pixmap oó dessiner (0 = Çcran)
		pos			->	dÇpart sur la ligne de base
		pstring		->	chaåne terminÇe par zÇro
		size		->	taille des caractäres
...
Pt DrawText (Pixmap *ppm, Pt pos, char *pstring, short size, ShowMode mode)
{
	...
	while ( (c = *pstring++, c != 0) )
	{
		DrawAccent(ppm, &pos, c, mode);				/* dessine le caractäre */
	}
...
/*
	Retourne le rectangle englobant une chaåne de caractäres.
...
/*
	Donne les caractÇristiques du mot suivant.
	Retourne la longueur Ö utiliser.
 */
...
/*
	Dessine un paragraphe de texte dans un rectangle, en drapeau Ö droite.
 */
...
void DrawParagraph (Pixmap *ppm, Rectangle rect, char *pstring, short size, ShowMode mode)
{
	...
	if ( *pstring != 0 )						/* texte pas entiärement affichÇ ? */
	{
...
/*
	Effectue quelques calculs pour afficher une ligne en Çdition.
 */
...
void CalcJustif (void)
{
	...
	cx  = chrect.p1.x;						/* curseur "|" au dÇbut */
	
	/*	Cherche dans la chaåne le premier caractäre Ö afficher. */
	...
	/*	Cherche le nombre de caractäres et la longueur Ö afficher. */
...
/*
	Affiche un texte pendant l'Çdition d'une ligne.
 */
...
void EditAff (void)
{
	...
	for ( i=begin ; i<lgchaine ; i++ )
	{
		...
		DrawAccent(0, &pos, pchaine[i], MODELOAD);		/* affiche un caractäre */
	}
...
/*
	Insäre un caractäre dans la ligne.
 */
...
short InsChar (char car)
{
	...
	if ( lgchaine >= lgmax-1 )  return -1;	/* chaåne pleine */
	...
	*(pchaine+curseur) = car;				/* met le caractäre dans la chaåne */
	lgchaine ++;
...
/*
	DÇtruit un caractäre dans la ligne.
 */
...
short DelChar (void)
{
	...
	if ( lgchaine == 0 )  return -1;		/* chaåne vide */
	if ( curseur == 0 )  return -1;			/* curseur au dÇbut */
...
/*
	...
	Si oui, retourne l'accent. Si non, retourne zÇro.
 */
...
/*
	...
	Si oui, retourne le nouveau caractäre. Si non, retourne zÇro.
 */
...
/*
	Enläve le curseur "|" si nÇcessaire.
 */
...
/*
	Modifie la chaåne en Çdition selon un ÇvÇnement clavier/souris.
	Retourne -1 en cas d'erreur.
	Retourne 1 si l'ÇvÇnement ne modifie pas le contenu.
	Retourne 0 si l'ÇvÇnement modifie le contenu.
 */
...
short EditEvent (short key, Pt pos)
{
	...
	{
		err = DelChar();				/* dÇtruit un caractäre */
	}
	...
	if ( ifaccent )						/* accent flottant en cours ? */
	{
		...
		if ( ifaccent )
		{
			...
			key = ifaccent;				/* remplace par lettre accentuÇe */
			goto ins;
		}
	}
	...
	if ( ifaccent )
	{
		key = ifaccent;					/* insäre l'accent flottant */
	}
	...
	if ( key >= 32 && key <= 127 ||
		 ...
		 key == '\n' )
	{
		err = InsChar((char)key);		/* insäre le caractäre frappÇ */
	}
...
/*
	Initialise la chaåne Ö Çditer, et place le curseur Ö la fin.
 */
...
/*
	Termine la chaåne Ö Çditer, et efface le curseur.
 */
...
short EditClose (void)
{
	...
	curseur = 0;					/* met le curseur au dÇbut */
	
	EditDraw();						/* affiche la chaåne */
	ClrCurs();						/* enläve le curseur "|" */
	
	pchaine = 0;					/* y'a plus de chaåne en Çdition */
